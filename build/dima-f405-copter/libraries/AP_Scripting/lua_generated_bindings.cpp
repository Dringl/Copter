...
int new_Cylinder_Status(lua_State *L) {
    ...
    memset(ud, 0, sizeof(Cylinder_Status));
...
int new_AP_EFI_Backend(lua_State *L) {
    luaL_checkstack(L, 2, "Out of stack");
    void *ud = lua_newuserdata(L, sizeof(AP_EFI_Backend *));
    memset(ud, 0, sizeof(AP_EFI_Backend *));
    luaL_getmetatable(L, "AP_EFI_Backend");
    lua_setmetatable(L, -2);
    return 1;
}
#endif // (AP_EFI_SCRIPTING_ENABLED == 1)
...
AP_EFI_Backend ** check_AP_EFI_Backend(lua_State *L, int arg) {
    void *data = luaL_checkudata(L, arg, "AP_EFI_Backend");
    return (AP_EFI_Backend **)data;
}
#endif // (AP_EFI_SCRIPTING_ENABLED == 1)
...
static int AP_EFI_get_backend(lua_State *L) {
    ...
    if (ud == nullptr) {
        return not_supported_error(L, 1, "efi");
    }
...
static int EFI_State_ecu_index(lua_State *L) {
    ...
    switch(lua_gettop(L)) {
        case 2: {
            const lua_Integer raw_data_2 = luaL_checkinteger(L, 2);
            luaL_argcheck(L, ((raw_data_2 >= MAX(0, 0)) && (raw_data_2 <= MIN(UINT8_MAX, UINT8_MAX))), 2, "ecu_index out of range");
            const uint8_t data_2 = static_cast<uint8_t>(raw_data_2);
            ud->ecu_index = data_2;
            return 0;
         }
...
static int EFI_State_throttle_position_percent(lua_State *L) {
    ...
    switch(lua_gettop(L)) {
        case 2: {
            const lua_Integer raw_data_2 = luaL_checkinteger(L, 2);
            luaL_argcheck(L, ((raw_data_2 >= MAX(0, 0)) && (raw_data_2 <= MIN(UINT8_MAX, UINT8_MAX))), 2, "throttle_position_percent out of range");
            const uint8_t data_2 = static_cast<uint8_t>(raw_data_2);
...
static int EFI_State_engine_load_percent(lua_State *L) {
    ...
    switch(lua_gettop(L)) {
        case 2: {
            const lua_Integer raw_data_2 = luaL_checkinteger(L, 2);
            luaL_argcheck(L, ((raw_data_2 >= MAX(0, 0)) && (raw_data_2 <= MIN(UINT8_MAX, UINT8_MAX))), 2, "engine_load_percent out of range");
            const uint8_t data_2 = static_cast<uint8_t>(raw_data_2);
